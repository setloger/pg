...
## Взгляд SRE на внутреннее устройство, тюнинг и траблшутинг

<br>

**Senior SRE** · PostgreSQL 16 / Patroni 3.x
...
## PostgreSQL 16 + Patroni 3.x — SRE-взгляд

<br>

| Область | Ключевое | Инструмент |
|---|---|---|
| Процессы | Process-per-connection | pgBouncer / Odyssey |
| Память | shared_buffers + work_mem | pg_settings |
| Надёжность | WAL + Checkpoint | pg_checksums |
| Очистка | Autovacuum tuning | pg_stat_user_tables |
| Безопасность | scram-sha-256 | pg_hba.conf |
| Наблюдаемость | pg_stat_statements | pg_stat_activity |
| HA | Leader Lock + DCS | Patroni + etcd |
| Трафик | REST API /primary | HAProxy |
